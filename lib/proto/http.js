...
module.exports = {
    check: async function check(link, opts, callback, attempts, additionalMessage) {
        ...
        // retry using puppeteer eg. for links that use cloudflare
        let retryWithPuppeteer = opts.retryWithPuppeteer || false;
        ...
        needle.head(url, options, async function (err, res) {
            ...
            await needle.get(url, options, async function (err, res) {
                ...
                if (retryOn429 && res && res.statusCode === 429 && attempts < retryCount) {
                    ...
                    // 403 is used by Cloudflare to prevent DDOS
                    // 999 is used by LinkedIn for unknown reasons
                    // Puppeteer can navigate the page and check the content in these edge cases
                    if (res && retryWithPuppeteer) {
                        const browser = await puppeteer.launch({ headless: true });
                        const page = await browser.newPage();
                        let statusCode;
                        try {
                            const response = await page.goto(link, { waitUntil: "networkidle0" });
                            statusCode = Number(response._status);
                        } catch (error) {
                        ...
                        await browser.close();
                        callback(null, new LinkCheckResult(opts, link, statusCode ?? 0, err));
                    } else {
                        if (additionalMessage) {
                            err = (err == null) ? additionalMessage : `${err} ${additionalMessage}`;
                        };
