...
describe('link-check', function () {
    ...
    it('should retry 3 times for 429 status codes', (done) => {
        ...
        linkCheck(
            baseUrl + '/later-custom-retry-count?successNumber=3',
            { retryOn429: true, retryCount: 3 },
            (err, result) => {
                expect(err).to.be(null)
                expect(result!.err).to.be(null)
                expect(result!.status).to.be('alive')
    ...
    it('should retry after the default delay on error', (done) => {
        laterCustomRetryCounter = 0
        linkCheck(
            baseUrl + '/error-retry-count?successNumber=4',
            { retryOnError: true, timeout: '500ms', fallbackRetryDelay: '0s' },
            (err, result) => {
                expect(err).to.be(null)
                expect(result!.err).to.be(null)
                expect(result!.status).to.be('alive')
